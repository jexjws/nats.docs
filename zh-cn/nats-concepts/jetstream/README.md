...
JetStream 基于文件的流会将消息持久化到磁盘。然而，虽然 JetStream 会同步地将文件写入刷新到操作系统，但在默认配置下，它不会立即将数据 `fsync` 到磁盘。服务器使用可配置的 `sync_interval` 选项，默认值为 2 分钟，用于控制服务器多久 `fsync` 一次数据。数据将在不晚于此间隔的时间内被 `fsync`。这对于应对操作系统故障（指操作系统的非正常退出，如断电，而不仅仅是 `nats-server` 进程本身的非正常退出或被杀死）时的持久性有重要影响：

在非复制设置中，操作系统故障可能导致数据丢失。客户端可能发布消息并收到确认，但数据可能尚未安全存储到磁盘。因此，在操作系统故障恢复后，服务器可能丢失了最近确认的消息。

在复制设置中，已发布的消息在成功复制到至少法定数量的服务器后才会被确认。然而，仅靠复制本身不足以保证针对多个系统性故障的最强持久性级别。
- 如果多个服务器同时故障，都是由于操作系统故障，并且在其数据被 `fsync` 之前，集群可能无法恢复最近确认的消息。
- 如果某个故障服务器由于操作系统故障而在本地丢失了数据，虽然极其罕见，但在某些事件组合下，它可能重新加入集群并与从未接收或持久化给定消息的节点形成新的多数。然后集群可能会继续处理不完整的数据，导致已确认的消息丢失。

设置较低的 `sync_interval` 会增加磁盘写入的频率，并减少潜在数据丢失的窗口，但会以性能为代价。此外，设置 `sync_interval: always` 将确保服务器在每条消息被确认之前都进行 `fsync`。这个设置与跨不同数据中心或可用区的复制结合使用，可提供最强的持久性保证，但性能最慢。

默认设置旨在在我们认为的典型生产部署场景（跨多个可用区）中平衡性能和数据丢失风险。

例如，考虑一个跨三个独立可用区部署的具有 3 个副本的流。要使流状态在节点间出现分歧，需要满足以下条件：
- 3 个服务器中的一个已经离线、隔离或分区。
- 第二个服务器的操作系统需要失败，以至于它丢失了仅在 3 个节点中的 2 个上可用的消息写入（因为它们未被 `fsync`）。
- 作为上述 3 个节点中 2 个的一部分的流领导者需要宕机或变得隔离/分区。
- 原始分区中未接收写入的第一个服务器从分区中恢复。
- 操作系统故障的服务器现在返回并与第一个服务器接触，但未与先前的流领导者接触。

最终，3 个节点中的 2 个将可用，具有写入的先前流领导者将不可用，一个服务器将由于操作系统故障而丢失了一些写入，另一个服务器由于先前的分区而从未见过这些写入。最后两个服务器然后可能形成多数并接受新的写入，本质上丢失了一些先前的写入。

重要的是，这是一个流状态可能出现分歧的故障条件，但在跨多个可用区部署的系统中，需要多个故障以正确的方式精确对齐。

对于这种故障的潜在缓解措施是，在已知大多数剩余服务器已接收到新写入之前，不要自动恢复操作系统故障的服务器进程，或者通过对等删除(peer-remove)崩溃的服务器并将其作为新的和被擦除的对等体接纳，并允许它从现有的健康节点通过网络恢复（尽管根据涉及的数据量，这可能成本高昂）。

对于将最小化损失作为绝对优先事项的用例，当然仍可配置 `sync_interval: always`，但请注意，这将对服务器范围的性能产生影响，可能影响吞吐量或延迟。对于生产环境，运营商应评估默认值是否适合其用例、目标环境、成本和性能要求。

或者，可以使用混合方法，其中现有集群仍在其默认 `sync_interval` 设置下运行，但添加一个配置为 `sync_interval: always` 的新集群，并利用服务器标签。然后可以通过使用[放置标签](streams.md#placement)指定流的放置，将此流的数据存储在这个更高持久性的集群上。
```
# 配置专门用于始终同步写入的集群。
...
创建一个专门放置在使用 `sync_interval: always` 的集群中的复制流，以确保仅对需要此持久性级别的流写入提供最强的持久性保证。
```
