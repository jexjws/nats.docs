...
在本次对比中，我们将介绍NATS、Apache Kafka、Rabbit、Apache Pulsar和gRPC。
...
| **Rabbit** | 至少10种维护者支持的客户端平台，超过50种社区支持的客户端类型。服务器支持以下平台：Linux、Windows NT。 |
...
| **Rabbit** | 通过发布/订阅实现流，以及通过 直接回复到... 功能实现服务。负载均衡可通过工作队列实现。应用程序必须通过多个主题将请求与回复相关联，以实现服务（请求-回复）模式。 |
...
| **Rabbit** | 通过虚拟主机（vhosts）支持多租户；不支持数据共享。 |
...
| **Rabbit** | TLS、SASL、用户名和密码，以及可插拔授权。 |
...
| **Rabbit** | 支持基于文件的持久化。Rabbit支持基于队列的语义（而非日志），因此无法进行消息回放。 |
...
| **Rabbit** | 通过联邦插件提供完整的数据复制集群支持。集群要求低延迟网络，且网络分区很少发生。 |
...
| **Rabbit** | Rabbit 支持集群，并通过联邦插件支持跨集群消息传播。客户端无需感知拓扑结构，可以连接到任何集群。服务器需要 Erlang VM 及相关依赖。 |
...
| **Rabbit** | 命令行工具（CLI）、一个基于插件的管理系统（包含仪表板）以及第三方工具。 |
...
| **Rabbit** | Rabbit 拥有许多插件，包括协议（MQTT, STOMP）、WebSockets 以及各种授权和认证插件。 |
